...
class TestDoodbaTasks(SharedTemplate):
    ...
    def test_resetdb(self, shared_project: Path, supported_odoo_version: float):
        ...
        with local.cwd(shared_project / "odoo" / "custom" / "src" / "odoo"):
    ...
    def test_start(self, shared_project: Path, supported_odoo_version: float):
        """Test the start task.
    ...
    def test_install_test(self, shared_project: Path, supported_odoo_version: float):
        ...
        if supported_odoo_version > 8:
            ...
            with local.cwd(
                shared_project / "odoo" / "custom" / "src" / "odoo" / "addons" / "utm"
            ):
    ...
    def test_test_tasks(self, shared_project: Path, supported_odoo_version: float):
        ...
        if supported_odoo_version >= 11:
            ...
            with local.cwd(shared_project / "odoo" / "custom" / "src" / "private"):
                generate_test_addon(
