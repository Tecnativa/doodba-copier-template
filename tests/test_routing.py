...
def test_multiple_domains(
    ...
    docker = DockerClient(compose_files=[f"{environment}.yaml"])
    run_copy(
        str(cloned_template),
    ...
    if is_traefik1:
        ...
        assert bad_response.url == f"https://main2.{base_domain}:443/insecure/path"
    else:
        # main0, no TLS
        if not is_traefik3:
    ...
    if not is_traefik3:
        ...
        for alt_num in range(2):
            ...
            assert response.ok
            assert response.url == f"https://main1.{base_domain}/web/database/selector"
            assert response.headers["X-Robots-Tag"] == "noindex, nofollow"
    ...
    bad_response = requests.get(f"http://missing.{base_path}", verify=not is_traefik1)
    assert bad_response.status_code == 404
