...
class TestDefaultSettings(SharedTemplate):
    def test_default_settings(
        ...
        Path(shared_project, "odoo", "auto", "addons").rmdir()
        Path(shared_project, "odoo", "auto").rmdir()
        git("add", ".")
    ...
    def test_pre_commit_autoinstall(
        ...
        with (shared_project / "odoo" / "custom" / "src" / "addons.yaml").open(
            "w"
        ) as fd:
        ...
        server_tools_git = (
            shared_project / "odoo" / "custom" / "src" / "server-tools" / ".git"
        )
