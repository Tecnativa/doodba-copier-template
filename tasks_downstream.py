...
def updatepot(
    ...
    if not module and not all and not repo:
        cur_module = _get_cwd_addon(Path.cwd())
        if not cur_module:
            raise exceptions.ParseError(
                msg="Odoo addon to update translation not found "
                "You must provide at least one of: -m {module}, be in the subdirectory of a module, --all or -r {repo}"
                " See --help for details."
            )
        module = cur_module
    ...
    cmd = (
        ...
        f"click-odoo-makepot --addons-dir {'/tmp/odoo/auto/addons' if not repo else '/tmp/odoo/custom/src/'+ repo}/"
    )
    ...
    glob = f"{PROJECT_ROOT}/odoo/custom/src/{ '*' if not repo else repo}/{ '*' if all or repo else module}/i18n/"
